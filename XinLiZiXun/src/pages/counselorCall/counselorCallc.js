import { showToast, formatNumber, formatTime1, sleep } from '../../utils/util.js'
...
Page({
  ...
  onLoad: function (options) {
    ...
    let self = this
    console.log(options)
    let {avatar, name, consultantId} = options
    ...
    // })
    let pageTitle = ''
    if (options.beCalling) { // 被叫 options.beCalling true
      // todo
  ...
  listenNetcallEvent() {
    ...
    app.globalData.emitter.on('syncDone', (data) => {
      ...
      //     // 设置开始时间的缓存
      //     console.log('同步完成之后 设置开始时间的缓存 设置开始时间的缓存 设置开始时间的缓存')

      //     wx.setStorageSync('turnOnTime' + self.data.consultantId, new Date().getTime())
      //     wx.setStorageSync('turnEndTime' + self.data.consultantId, new Date().getTime() + 1000 * 60 * wx.getStorageSync('consultingLengthTime' + self.data.consultantId))

      //     console.log('wx.setStorageSync(turnOnTime)', new Date().getTime())
      //     console.log('wx.setStorageSync(consultingLengthTime)', 1)
      //     console.log('wx.setStorageSync(turnEndTime)', formatTime1(new Date(new Date().getTime() + 1000 * 60 * wx.getStorageSync('consultingLengthTime' + self.data.consultantId))))
      ...
      if (!self.netcallTimeTimer) {
        self.netcallTimeTimer = setInterval(() => {
          ...
          // console.log(wx.getStorageSync('realTime'), 'wx.getStorageSync(realTime)')
          // console.log(wx.getStorageSync('turnEndTime' + self.data.consultantId), 'turnEndTime v turnEndTime turnEndTime')
          // if (self.data.callee) {
          //   if (wx.getStorageSync('turnEndTime') < +new Date()) {
          //     // init
          //     clearInterval(self.netcallTimeTimer)
          //     wx.setStorageSync('consultingLengthTime' + self.data.consultantId, null) // 要付钱了
          ...
          //     showToast('text', `通话时长已到，窗口3秒后关闭`, { duration: 3000 })
          //     sleep(3).then( res => {
          //       self.hangupHandler()
          //     })
          //   }
    ...
    app.globalData.emitter.on('hangup', (data) => {
      ...
      // if (this.data.callee && this.data.clearBusy) {
      //   consultantEndConnect().then(res => {
      //     console.log('咨询师挂断了 咨询师挂断了 咨询师挂断了 咨询师挂断了')
      //   })
      // }
