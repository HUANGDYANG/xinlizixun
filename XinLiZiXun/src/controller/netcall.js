import NetcallWeixin from '../utils/NIM_Web_Netcall_weixin_v6.2.0'
import NIM from '../utils/NIM_Web_NIM_weixin_v6.2.0'
...
export default class NetcallController {
  ...
  bindNetcallEvent() {
    ...
    this.netcall.on('beCalling', (data) => {
      // {caller,cid,type}
      console.log('beCalling beCalling beCalling beCalling response', data)
      console.log('onTheCall', data.onTheCall)
      let pages = getCurrentPages()
      let currentPage = pages[pages.length - 1]
      if (app.globalData.toVideoCall) return
      if (currentPage.route.includes('videoCall') === false) { // 不在多人通话中，才提示
        if (!currentPage) {
          wx.navigateTo({
            url: `/pages/videoCall/videoCall?beCalling=true&caller=${data.caller}&cid=${data.cid}&type=${data.type}`
          })
          app.globalData.isPushBeCallPage = true
          app.globalData.toVideoCall = true
          return
        }
        let netcallGroupCallInfo = app.globalData.netcallGroupCallInfo
        let pages = getCurrentPages()
        let currentPage = pages[pages.length - 1]
