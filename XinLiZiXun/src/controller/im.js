...
export default class IMController {
  constructor(headers) {
    ...
    app.globalData.nim = NIM.getInstance({
      ...
      // 私有化配置文件
      privateConf: app.globalData.ENVIRONMENT_CONFIG.openPrivateConf
        ? app.globalData.ENVIRONMENT_CONFIG.privateConf
        : '',
      // 同步完成
   ...
   */
  onDisconnect(error) {
    console.log(this.frist, 'this.frist')
    ...
    if (error) {
      switch (error.code) {
        ...
        case 302:
          ...
          if (frist) {
            let { getMemberImInfo } = api
            if (app.globalData.userInfo) {
              getMemberImInfo().then(res => {
                if (res) {
                  console.log(res, '实时通讯 实时通讯')
                  console.log(res, '实时通讯 实时通讯 onshow')
                  new IMController({
                    // 实时通讯
